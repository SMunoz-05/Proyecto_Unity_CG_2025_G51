...
public class Utilidades
{
    ...
    public static bool SaveDataStudentIngenieria(List<Estudiante> listaEIngenieria)
    {
        try
        {
            string jsonString = JsonUtility.ToJson(new EstudianteListIngWrapper { estudiantes = listaEIngenieria }, true);
            string folderPath = Application.streamingAssetsPath;
            if (!Directory.Exists(folderPath))
            {
                Directory.CreateDirectory(folderPath);
            }
            string filePath = Path.Combine(folderPath, "datosEstudianteIngenieria.json"); 
            File.WriteAllText(filePath, jsonString);
            Debug.Log(" Archivo JSON guardado correctamente en: " + filePath);
            return true;
        }
...
[Serializable]
public class EstudianteListIngWrapper
{
